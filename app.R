...
ui <- fluidPage(
  ...
  tags$head(tags$style(HTML(paste0("
    ...
    .trend-card {
      border-radius:6px; padding:12px 15px; font-size:0.88rem; line-height:1.6;
    }
  ...
  sidebarLayout(
    ...
    mainPanel(width = 9,
              ...
              conditionalPanel("input.tab == 'overview'",
                               ...
                               div(style = "margin:4px 0 2px;", class = "atext", textOutput("data_note")),
                               br(),
                                 ...
                                 column(5,
                                        div(class = "info-box amber",
                                            ...
                                            tags$p(style = "margin:6px 0 0;",
                                                   "The NDI compares the site with the highest mean PM2.5 to the site ",
                                                   "with the lowest, expressed as a ratio. An NDI of 2 means the most ",
                                                   "polluted site is twice as bad as the cleanest site in the network."
                                            ),
              ...
              conditionalPanel("input.tab == 'spatial'",
                               fluidRow(
                                 column(4,
                                        value_box("Congestion vs Remote \u0394PM2.5",
                                                  textOutput("concentration_diff_value"),
                                                  showcase = bs_icon("geo-alt-fill"), class = "bg-warning")
                                 ),
                                 column(4,
                                 ...
                                 column(6,
                                        ...
                                        div(class = "info-box amber",
                                            ...
                                            tags$p(style = "margin:6px 0 0;",
                                                   ...
                                                   tags$code("dist \u2264 5 km"), " are averaged (congestion mean). ",
                                                   "Readings from stations with ", tags$code("dist > 5 km"),
                                 ...
                                 column(6,
                                        ...
                                        div(class = "info-box green",
                                            ...
                                            tags$p(style = "margin:6px 0 0;",
                                                   ...
                                                   "Spatial correlation = Pearson r between pairwise inter-site distance ",
                                                   "and absolute PM2.5 difference."
                                            )
                                        )
                                 )
                               ),
                               ...
                               fluidRow(
                                 ...
                                 column(5,
                                        div(class = "info-box",
                                            ...
                                            tags$p(style = "margin:6px 0 0;",
                                                   "Each point = one monitoring station. ",
                                                   tags$b("Downward slope"), " \u2192 closer = more polluted. ",
                               ...
                               tags$p(style = "color:#888;font-size:0.82rem;font-style:italic;text-align:center;margin-top:4px;",
                                      "Distance from each AQS site centroid to nearest congestion zone boundary (m \u2192 km)."),
                               tags$hr(),
                               tags$h4(class = "sec-hdr",
...
server <- function(input, output, session) {
  ...
  output$county_selector <- renderUI({
    ...
    tagList(
      ...
      checkboxGroupInput("selected_counties", NULL,
                         choices = setNames(ids, nms), selected = ids)
    )
  })
  ...
  spatial_interpolation_analysis <- reactive({
    ...
    mn_d   <- mean(dmat[upper.tri(dmat)], na.rm = TRUE)
    ...
    list(mean_distance = mn_d, concentration_cv = cv,
  ...
  scatter_lm_stats <- reactive({
    sm <- site_means() %>% st_drop_geometry() %>% filter(!is.na(dist_km))
    if (nrow(sm) < 3) return(NULL)
    mod <- lm(mean_pm25 ~ dist_km, data = sm)
    s   <- summary(mod)$coefficients
    ci  <- confint(mod, "dist_km")
    list(slope = s["dist_km","Estimate"], se_slope = s["dist_km","Std. Error"],
         ci_lo = ci[1], ci_hi = ci[2], p_value = s["dist_km","Pr(>|t|)"],
         r2 = summary(mod)$r.squared, n_sites = nrow(sm))
  })
  ...
  output$concentration_diff_value <- renderText({
    r <- spatial_analysis()
    if (is.na(r$diff_value)) "N/A" else paste0(round(r$diff_value,2)," \u03bcg/m\u00b3")
  })
  output$hotspot_count_value <- renderText({
    r <- spatial_hotspot_analysis()
    if (!is.null(r$error)) "N/A" else paste0(r$n_hotspots," / ",r$n_total," sites")
  })
  output$n_sites_value <- renderText({ paste0(nrow(site_means())," sites") })
  
  output$spatial_summary_sentence <- renderText({
    r <- spatial_analysis()
    if (is.na(r$diff_value)) return("No valid spatial data for the selected filters.")
    paste0("Congestion mean (",r$n_cong," obs, dist \u2264 50 m): ",
           round(r$congestion_avg,2)," \u03bcg/m\u00b3. Remote mean (",
           r$n_remote," obs, dist > 50 m): ",round(r$remote_avg,2),
           " \u03bcg/m\u00b3. \u0394 = ",round(r$diff_value,2)," \u03bcg/m\u00b3.")
  })
  output$interpolation_insight_text <- renderText({
    r <- spatial_interpolation_analysis()
    if (!is.null(r$error)) return(r$error)
    het <- if (r$concentration_cv>0.3) "high" else if (r$concentration_cv>0.15) "moderate" else "low"
    dep <- if (abs(r$spatial_correlation)>0.3) "moderate" else if (abs(r$spatial_correlation)>0.1) "weak" else "minimal"
    paste0("Inter-site distance: ",round(r$mean_distance/1000,1)," km. CV: ",
           round(r$concentration_cv,2)," (",het," heterogeneity). Spatial r: ",
           round(r$spatial_correlation,2)," (",dep," dependency) across ",r$n_sites," sites.")
  })
  output$hotspot_analysis_text <- renderText({
    r <- spatial_hotspot_analysis()
    if (!is.null(r$error)) return(r$error)
    sp <- if (r$hotspot_percentage>25) "widespread" else if (r$hotspot_percentage>10) "moderate" else "limited"
    paste0("Threshold: mean + 1 SD = ",round(r$threshold,2)," \u03bcg/m\u00b3. ",
           r$n_hotspots," of ",r$n_total," sites (",r$hotspot_percentage,
           "%) flagged \u2014 ",sp," clustering.")
  })
  ...
  output$scatter_trend_card <- renderUI({
    ...
    p_fmt <- if (s$p_value<0.001) "< 0.001" else if (s$p_value<0.01) "< 0.01" else
    ...
    body <- if (sig && s$slope<0)
      paste0("Each +1 km \u2192 PM2.5 decreases by ",round(abs(s$slope),3)," \u03bcg/m\u00b3.")
    else if (sig && s$slope>0)
      paste0("Each +1 km \u2192 PM2.5 increases by ",round(s$slope,3)," \u03bcg/m\u00b3.")
    else "Distance does not significantly predict PM2.5 here."
  ...
  output$ts_plot <- renderPlotly({
    ...
    p <- ggplot(data, aes(x = datetime, y = display_value,
      ...
      scale_x_datetime(
        limits = c(min(data$datetime, na.rm=TRUE), max(data$datetime, na.rm=TRUE)),
  ...
  output$monthly_trend <- renderPlotly({
    ...
    mod  <- lm(mean_pm25 ~ time_index, data=data)
    pred <- predict(mod, interval="confidence")
    data$fit <- pred[,"fit"]; data$lwr <- pred[,"lwr"]; data$upr <- pred[,"upr"]
    data <- data %>%
      mutate(label_pt  = paste0("Month: ",format(date_month,"%b %Y"),
                                "<br>Mean PM2.5: ",round(mean_pm25,2)," \u03bcg/m\u00b3"),
             label_fit = paste0("Trend: ",round(fit,2)," \u03bcg/m\u00b3"))
    p <- ggplot(data, aes(x=date_month)) +
      geom_ribbon(aes(ymin=lwr, ymax=upr), fill=CLR_RED, alpha=0.15) +
      geom_line(aes(y=fit, text=label_fit), color=CLR_RED, linewidth=1) +
      geom_point(aes(y=mean_pm25, text=label_pt), color=CLR_BLUE, size=2.8) +
  ...
  output$trend_text <- renderText({
    data <- monthly_data()
    if (nrow(data) < 2) return("Need \u2265 2 months of data.")
    ct <- summary(lm(mean_pm25 ~ time_index, data=data))$coefficients
    sl <- ct["time_index","Estimate"]; pv <- ct["time_index","Pr(>|t|)"]
    if (pv < 0.05)
      paste0("Linear trend: ",if(sl>0)"significant increasing" else "significant decreasing",
             " (slope = ",format(abs(sl),digits=3)," \u03bcg/m\u00b3/month, p < 0.05).")
    else "Linear trend: No statistically significant trend (p \u2265 0.05)."
  })
  ...
  output$spatial_scatter <- renderPlot({
    ...
    sm  <- sm %>% mutate(
      is_hot = aqs_id_full %in% (if (!is.null(hot$hotspot_ids)) hot$hotspot_ids else character())
    )
    ggplot(sm, aes(x=dist_km, y=mean_pm25)) +
      ...
      scale_color_manual(values=c("FALSE"=CLR_BLUE,"TRUE"=CLR_RED),
                         labels=c("FALSE"="Normal","TRUE"="Hotspot"), name=NULL) +
      scale_size_manual(values=c("FALSE"=3,"TRUE"=4.5), guide="none") +
      labs(title="PM2.5 vs Distance to Congestion Zone",
           x="Distance to Congestion Zone (km)", y="Mean PM2.5 (\u03bcg/m\u00b3)") +
      theme_minimal(base_size=12) +
      theme(plot.title=element_text(face="bold",size=12), legend.position="bottom")
  })
  
  # ── Leaflet map ──────────────────────────────────────
  # Strategy: keep RdYlGn fill for PM2.5 color scale (bottom-right legend).
  # Add a SECOND larger transparent circle behind each marker whose
  # stroke color = CLR_RED (hotspot) or CLR_BLUE (normal).
  # This "ring" is visually distinct and matches the bottom-left legend.
  output$spatial_map <- renderLeaflet({
    ...
    sm <- sm %>%
      mutate(
        ...
        fill_radius  = pmax(sqrt(mean_pm25) * 2, 4),   # inner filled circle
        ring_radius  = pmax(sqrt(mean_pm25) * 2, 4) + 4, # outer ring slightly larger
        popup_text   = paste0(
          if_else(is_hot,
          ...
          "Distance to zone: ", round(dist_km * 1000, 0), " m"
        )
      )
    ...
    leaflet(sm) %>%
      ...
      # ── Layer 1: outer ring circles (classification color, no fill) -------------------------------------------------------
      addCircleMarkers(
        radius      = ~ring_radius,
        color       = ~ring_color,
        weight      = 3,
        fill        = FALSE,        # transparent fill — only the stroke ring shows
        opacity     = 0.9,
        popup       = ~popup_text
      ) %>%
      
      # ── Layer 2: inner filled circles (RdYlGn PM2.5 color) ──
      addCircleMarkers(
        radius      = ~fill_radius,
        color       = "white",      # thin white border on the fill circle
        weight      = 1,
      ...
      # PM2.5 color scale legend (bottom-right)
      addLegend(
      ...
      # Site classification ring legend (bottom-left)
      addLegend(
        position = "bottomleft",
        ...
        title    = paste0(
          ...
          "<small style='font-we1ight:400;color:#555;'>",
          "Ring color around each circle</small>"
        ),
        opacity  = 0.9
      )
  })
